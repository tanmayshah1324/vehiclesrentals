...
const AdBanner = ({ type = 'horizontal', className = '' }) => {
    ...
    React.useEffect(() => {
        const fetchAds = async () => {
            try {
                const data = await apiService.ads.getAll();
    ...
    if (type === 'horizontal') {
        return (<div className={`relative group overflow-hidden rounded-2xl shadow-lg bg-gradient-to-r ${ad.color} text-white ${className}`}>
        <div className="absolute top-2 right-2 z-10">
          <button onClick={() => setIsVisible(false)} className="p-1 rounded-full bg-black/20 hover:bg-black/40 transition-colors" title="Close ad">
            <X className="w-4 h-4"/>
          </button>
        </div>
    ...
    return (<div className={`relative group overflow-hidden rounded-xl shadow-md bg-gradient-to-br ${ad.color} text-white p-6 ${className}`}>
      <div className="absolute top-2 right-2 z-10">
        <button onClick={() => setIsVisible(false)} className="p-1 rounded-full bg-black/20 hover:bg-black/40 transition-colors">
          <X className="w-4 h-4"/>
        </button>
      </div>
      ...
      <img src={ad.image} alt={ad.title} className="w-full h-32 object-cover rounded-lg mb-4 group-hover:scale-105 transition-transform duration-500"/>

...
const VehiclesPage: React.FC = () => {
  ...
  const getInitialType = () => {
    ...
    if (location.pathname.includes('/cars')) return 'car';
    if (location.pathname.includes('/bikes')) return 'bike';
    return 'all';
  };
  ...
  const fetchVehicles = async () => {
    ...
    try {
      ...
      setError('Could not connect to the backend server. Please make sure the server is running using "npm run dev:all"');
      setLoading(false);
    }
  };
  ...
  const filteredVehicles = vehicles.filter((vehicle) => {
    ...
    if (rentalType) {
      if (rentalType === 'hourly' && !vehicle.price.hourly) return false;
      if (rentalType === 'daily' && !vehicle.price.daily) return false;
      if (rentalType === 'weekly' && !vehicle.price.weekly) return false;
    }
  ...
  return (
    <div className="container px-4 py-12 mx-auto">
        ...
        <div className="flex flex-col space-y-4 md:flex-row md:items-center md:space-y-0 md:space-x-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="block w-full py-2 pl-10 pr-3 border border-gray-300 rounded-md shadow-sm dark:border-gray-600 focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
            />
          </div>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-100 dark:border-red-900/30 px-4">
